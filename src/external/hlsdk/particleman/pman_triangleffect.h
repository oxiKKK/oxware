...
#define CULL_PVS					( 1 << 3 )
#define CULL_FLAGS					(CULL_PVS | CULL_FRUSTUM_POINT | CULL_FRUSTUM_PLANE | CULL_FRUSTUM_SPHERE)

#define LIGHT_NONE					( 1 << 4 )
#define LIGHT_COLOR					( 1 << 5 )
#define LIGHT_INTENSITY				( 1 << 6 )
#define LIGHT_FLAGS					(LIGHT_NONE | LIGHT_INTENSITY | LIGHT_COLOR)
...
#define RENDER_FLAGS				(RENDER_FACEPLAYER | RENDER_FACEPLAYER_ROTATEZ)
...
public:
	...
	{
		m_iRenderFlags &= ~LIGHT_FLAGS;
		m_iRenderFlags |= iFlag;
	}

	void   SetCullFlag(int iFlag)
	{
		m_iRenderFlags &= ~CULL_FLAGS; // clear first
		m_iRenderFlags |= iFlag;
	}
